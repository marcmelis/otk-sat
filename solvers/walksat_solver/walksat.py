...
def WalkSat(Object):
        ...
        while True:
            ...
            for flips in xrange(max_flips):
                if check_solution(assignment, formula):
                    return assignment
                C = a clause of F not satisfied by assignment
                S = set of variables that apear in C
                B = min({broken(p, F, I) | p in S})
                if b > 0 and with probability W:
                    p = variable of S
                else:
                    p = variable of S s.t. broken(p, F, I) = B
    ...
    def lit_clause_struct(formula, n_vars):
        pos = [[] for _ in repeat(None, n_vars + 1)]
        neg = [[] for _ in repeat(None, n_vars + 1)]
        index_c = 1
        for clause in formula:
            index_l = 1
            for literal in clause:
    ...
    def main():
        ...
        print "Pos: ", pos_list
        print "Neg: ", neg_list
        solution = walksat(formula, n_vars)
        print 's SATISFIABLE'
